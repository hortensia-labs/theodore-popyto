...
/**
 * Preview a URL by calling the citation linker API
 */
export async function previewUrl(url: string): Promise<{
  ...
  try {
    ...
    const previewData: PreviewUrlData = {
    ...
    return {
      success: false,
      error: error instanceof Error ? error.message : 'Unknown error previewing URL',
    };
  }
}
